...
def initLogging(thisLevel, funcLen="24"):
    log_format = (
        "[%(asctime)s] %(funcName)-" + str(funcLen) + "s %(levelname)-8s %(message)s"
    )
    if thisLevel == 20:
...
if __name__ == "__main__":
    argumentparser = argparse.ArgumentParser(description="Mimir Terminal Frontend")
    ...
    argumentparser.add_argument(
        "--folder", action="store", help="config file", type=str, required=True
    )
    argumentparser.add_argument(
        "--config", action="store", help="config file", type=str, default=None
    )
    argumentparser.add_argument(
        "--disableAutoFind",
        action="store_true",
    ...
    run(baseDir=args.folder, config=args.config, autofind=(not args.disableAutoFind))
